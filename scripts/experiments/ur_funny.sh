...
for fold in "${FOLDS[@]}"; do
  for lr in "${BASE_LRS[@]}"; do
    for wd in "${BASE_WDS[@]}"; do
      if do_train; then
        run_train --config ./configs/FactorCL/URFunny/release/VT/unimodal_video.json --default_config "${DEFAULT_CONFIG}" --fold 0 --lr "${lr}" --wd "${wd}" --validate_with accuracy
      fi
      ...
      if do_show && [[ "${fold}" == "0" ]]; then
        run_show --config ./configs/FactorCL/URFunny/release/VT/unimodal_text.json --default_config "${DEFAULT_CONFIG}" --fold 0 --lr "${lr}" --wd "${wd}" --validate_with accuracy
      fi
    done
  done
done
...
#        if do_train; then
#          run_train --config "./configs/FactorCL/URFunny/release/VT/${cfg}.json" --default_config "${DEFAULT_CONFIG}" --fold 0 --lr "${lr}" --wd "${wd}" --validate_with accuracy
#        fi
#        if do_show && [[ "${fold}" == "0" ]]; then
#          run_show --config "./configs/FactorCL/URFunny/release/VT/${cfg}.json" --default_config "${DEFAULT_CONFIG}" --fold 0 --lr "${lr}" --wd "${wd}" --validate_with accuracy
#        fi
#      done
#    done
#  done
...
#          if do_train; then
#            run_train --config "./configs/FactorCL/URFunny/release/VT/${cfg}.json" --default_config "${DEFAULT_CONFIG}" --fold 0 --lr "${lr}" --wd "${wd}" --l "${l}" --cls mlp --validate_with syn_accuracy
#          fi
#          if do_show && [[ "${fold}" == "0" ]]; then
#            run_show --config "./configs/FactorCL/URFunny/release/VT/${cfg}.json" --default_config "${DEFAULT_CONFIG}" --fold 0 --lr "${lr}" --wd "${wd}" --l "${l}" --cls mlp --validate_with syn_accuracy
#          fi
#        done
#      done
#    done
