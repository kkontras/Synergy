...
def compute_correct_intersections_from_preds(probes, y_val):
    ...
    intersections = {
        ...
        "z1 ∩ z_tilde": correct["z1"] & correct["z_tilde"],
        "z2 ∩ z_tilde": correct["z2"] & correct["z_tilde"],
        "all 3": correct["z1"] & correct["z2"] & correct["z_tilde"],
    }
...
def tsne_plot(z1, z2, z_tilde, y, perplexity=30, separate_tsne=False):
    ...
    if separate_tsne:
        z_t_e = TSNE(n_components=2, perplexity=perplexity).fit(z_tilde)
    else:
        z_t_e = ts1.transform(z_tilde)
    ...
    plt.tight_layout()
    fig.savefig("tsne_embeddings.png", dpi=300, bbox_inches='tight')
    plt.show()
...
def main():
    ...
    train_dict = {
        ...
        "z_tilde":  features_train["~z"]
    }
    val_dict = {
        "z1":        features_val["z1"],
        "z2":        features_val["z2"],
        "z_tilde":  features_val["~z"]
    }
