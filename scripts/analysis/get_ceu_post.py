...
def main(config_path, default_config_path, args):
    ...
    val_results = validator.get_results(set="Validation", print_results=True)
    ...
    targets = None
    try:
        preds = val_results["total_preds"]["combined"]
    except Exception:
        pass
    try:
        targets = val_results["total_preds_target"]
    except Exception:
        pass
    ...
    out_test = _to_jsonable(out_test)
    ...
    import pickle
    if os.path.exists(OUTPUT_JSON_val):
        with open(OUTPUT_JSON_val, "rb") as f:
            all_results_val = pickle.load(f)
    if os.path.exists(OUTPUT_JSON_test):
        with open(OUTPUT_JSON_test, "rb") as f:
            all_results_test = pickle.load(f)



    reform_fold = int(args.fold)
    all_results_val["folds"][reform_fold+offset] = out_val
    all_results_test["folds"][reform_fold+offset] = out_test

    with open(OUTPUT_JSON_val, "wb") as f:
        pickle.dump(all_results_val, f, protocol=pickle.HIGHEST_PROTOCOL)
    with open(OUTPUT_JSON_test, "wb") as f:
        pickle.dump(all_results_test, f, protocol=pickle.HIGHEST_PROTOCOL)
